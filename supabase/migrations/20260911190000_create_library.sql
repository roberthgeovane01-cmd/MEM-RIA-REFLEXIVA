-- Fase 2 — Biblioteca. See docs/DATA_MODEL.md ("Fase 2 — Biblioteca") and
-- docs/specs/MR-03-biblioteca.md / MR-04-adicionar-conteudo.md for the
-- functional spec these fields and rules come from.
...
create table public.library_items (
  ...
  -- Only the states Fase 2 (storage) and the trivial TXT/Markdown text
  -- extraction it does inline can actually reach. Fase 3 introduces the
  -- full async pipeline and the remaining states from docs/DATA_MODEL.md.
...
-- One row per stored file version. library_items.title/description/etc are
-- editable without ever touching the original file (docs/SECURITY.md: the
-- original is preserved and immutable by processing/metadata edits).
...
-- Private bucket for original documents. No public URLs — access only via
-- authenticated download or short-lived signed URLs (docs/SECURITY.md).
--
-- NOTE: Lovable Cloud's managed migrations reject `insert into
-- storage.buckets` directly — the bucket was created through its own
-- storage tool instead (bucket id/name "library-originals", public=false).
-- If you're applying this migration with the Supabase CLI against a plain
-- Supabase project, uncomment the statement below.
-- insert into storage.buckets (id, name, public)
-- values ('library-originals', 'library-originals', false)
-- on conflict (id) do nothing;

-- Storage object path convention: {owner_id}/{library_item_id}/{filename}.
-- storage.foldername(name) splits the path into an array of folder
-- segments, so [1] is the owner_id segment.
