...
-- constraint). 'processed' isn't in the new enum below (renamed to
-- 'completed'), so the order matters both ways: drop the old constraint
-- first (CHECK is validated immediately, and the old one would reject
-- 'completed'), rename the data while nothing is constraining it, THEN add
-- the new constraint — adding it any earlier would itself fail, since
-- existing 'processed' rows aren't valid under an enum that doesn't have
-- 'processed' in it.
